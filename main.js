...
let y = 2 * height / 3, x = width / 2, dy = 1+(parseInt(speed.value))*18/100, dx = 1+parseInt(speed.value)*20/100;
console.log(dx+"js"+dy);
let lives = 3;
...
function draw() {
    ...
    drawBricks();
    if(dy<0)
    dy = -(1+(parseInt(speed.value))*18/100);
    else
    dy=1+(parseInt(speed.value))*18/100;
    if(dx<0)
     dx = -(1+parseInt(speed.value)*40/100);
     else
     dx=1+parseInt(speed.value)*40/100;
